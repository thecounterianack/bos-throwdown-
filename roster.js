const ROSTER = [
  ...
  { id:'xackley', name:'X-Ackley', rung:8, hp:118, speed:6, reach:74, light:8, heavy:13, ai:0.58, weapon:'katana',
    taunt:"Hope you're comfortable — this is getting recorded." },
  { id:'tsspectre', name:'T.S. The Spectre', rung:9, hp:122, speed:7, reach:76, light:8, heavy:13, ai:0.64, weapon:'scythe',
    taunt:"Every bar I drop hits harder than this fist." },
  { id:'wraith', name:'Wraith', rung:10, hp:140, speed:7, reach:80, light:10, heavy:16, ai:0.78, weapon:'chainscythe',
...
function getById(id){
  return ROSTER.find(c=>c.id===id) || SHADOW_ROSTER.find(c=>c.id===id);
}
...
function buildLadder(pickedId){
  ...
  ROSTER.forEach(c=>{
    if(c.id === pickedId){
      ladder.push({ ...c, id:'mirror_'+c.id, name:c.name+' (Mirror)', mirror:true, sourceId:c.id,
        hp:Math.round(c.hp*1.05), light:Math.round(c.light*1.05), heavy:Math.round(c.heavy*1.05),
